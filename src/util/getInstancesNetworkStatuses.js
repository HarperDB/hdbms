const getInstanceNetworkStatus = async (instance, queryHarperDB) => {
  ...
  return {
    ...instance,
    fabric: {
      inbound_connections: inbound_connections.filter((c) => c.host_address.indexOf('::ffff') === -1),
      outbound_connections: outbound_connections.filter((c) => c.host_address.indexOf('::ffff') === -1),
