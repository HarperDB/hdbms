  ...
  try {
    ...
    const compute = computeProducts.find(
      (p) =>
        ...
        ((!instance.compute_subscription_id && !p.value.compute_subscription_id) || p.value.compute_subscription_id === instance.compute_subscription_id)
    );
    ...
    const storage = instance.is_local
      ...
      : storageProducts.find(
          (p) =>
            ...
            ((!instance.storage_subscription_id && !p.value.storage_subscription_id) || p.value.storage_subscription_id === instance.storage_subscription_id)
        );
