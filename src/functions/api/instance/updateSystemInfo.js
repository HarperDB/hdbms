...
export default async ({
  ...
  const memoryStatus = skip.includes('memory') ? cachedSystemInfo.memoryStatus : freeMemory / totalMemory < 0.1 ? 'danger' : freeMemory / totalMemory < 0.25 ? 'warning' : 'success';

  // DISK
  const totalDisk = skip.includes('disk') ? parseFloat(cachedSystemInfo.totalDisk) : result.system.platform === 'darwin' ? result.disk.size[0].size / B2GB1000 : !isLocal ? result.disk.size.find(disk => disk.mount === '/home/ubuntu/hdb').size / B2GB1024 : result.disk.size[0].size / B2GB1024;
  const usedDisk = skip.includes('disk') ? parseFloat(cachedSystemInfo.usedDisk) : result.system.platform === 'darwin' ? result.disk.size.reduce((a, b) => a + b.used, 0) / B2GB1000 : !isLocal ? result.disk.size.find(disk => disk.mount === '/home/ubuntu/hdb').used / B2GB1024 : result.disk.size.reduce((a, b) => a + b.used, 0) / B2GB1024;
  ...
  const cpuLoad = skip.includes('cpu') ? parseFloat(cachedSystemInfo.cpuLoad) : result.cpu.currentLoad.currentLoad || result.cpu.currentLoad.currentload || 0;
  const cpuStatus = skip.includes('cpu') ? cachedSystemInfo.cpuStatus : cpuLoad > 90 ? 'danger' : cpuLoad > 75 ? 'warning' : 'success';

  // NETWORK
  const networkTransferred = skip.includes('network') ? parseFloat(cachedSystemInfo.networkTransferred) : result.network.stats.reduce((a, b) => a + b.txBytes, 0) / B2GB1000;
  const networkReceived = skip.includes('network') ? parseFloat(cachedSystemInfo.networkReceived) : result.network.stats.reduce((a, b) => a + b.rxBytes, 0) / B2GB1000;
  const networkLatency = skip.includes('network') ? parseFloat(cachedSystemInfo.networkLatency) : result.network.latency.ms;
  const networkLatencyStatus = skip.includes('network') ? parseFloat(cachedSystemInfo.networkLatencyStatus) : networkLatency > 1000 ? 'danger' : networkLatency > 500 ? 'warning' : 'success';
  const systemInfo = {
