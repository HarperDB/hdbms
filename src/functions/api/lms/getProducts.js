    ...
    return appState.update((s) => {
      s.products = {
        ...
        wavelength_compute: buildRadioSelectProductOptions(
          response.find((p) => p.id === PRODUCT_MAP[config.env].wavelength_compute)?.plans.filter((p) => !p.metadata.prepaid) || []
        ),
        akamai_compute: buildRadioSelectProductOptions(response.find((p) => p.id === PRODUCT_MAP[config.env].akamai_compute)?.plans.filter((p) => !p.metadata.prepaid) || []),
