...
const checkClusterStatus = async ({ auth, url }) => {
	...
	const config_cluster_port =
		clusterEngine === 'nats' ? config.clustering.hubServer.cluster.network.port : config.CLUSTERING_PORT;
	const node_name = clusterEngine === 'nats' ? config.clustering.nodeName : config.NODE_NAME;

	const is_ready =
		!!is_enabled && !!cluster_user && !!config_cluster_user && !!cluster_role && !!config_cluster_port && !!node_name;
