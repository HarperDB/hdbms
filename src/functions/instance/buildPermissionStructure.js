...
const buildCurrent = ({ extantTablePermissions, attributes, showAttributes }) => ({
  ...
  attribute_permissions: attributes
    .filter(() => showAttributes || extantTablePermissions?.attribute_permissions?.some((att) => att.read || att.insert || att.update))
    .map((a) => {
      const extantAttributePermissions = extantTablePermissions?.attribute_permissions?.find((att) => att.attribute_name === a);

      return {
        attribute_name: a,
        read: extantAttributePermissions ? extantAttributePermissions.read : extantTablePermissions?.attribute_permissions.length ? false : extantTablePermissions?.read || false,
