...
function CardEdit({
      ...
      if (!card || !expire || !cvc || !postalCode || !line1 || !state || !city || !country) {
        ...
        if (payload.error) {
          ...
          if (response.error) {
            ...
            if (window.Kmq) window.Kmq.push(['record', 'added credit card - billing page']);
            setFormState({
  ...
  return <ErrorBoundary onError={(error, componentStack) => addError({
    ...
    customerId
      ...
      {formState.processing ? <FormStatus height={formStateHeight} status="processing" header="Adding Card To Your Account" subhead="The Credit Schnauzer is securely contacting Stripe." /> : formState.success ? <FormStatus height={formStateHeight} status="success" header="Success!" subhead="Credit Card was successfully added to your account." /> : formState.error ? <FormStatus height={formStateHeight} status="error" header={formState.error} subhead="Please try again" /> : <Card>
          <CardBody>
            {badCard && <BadCard />}
            <CreditCardForm setFormData={setFormData} formData={formData} />
            <hr className="my-2" />
            <Row>
              {customerCard && <Col sm="6">
                  <Button id="editCard" onClick={() => setEditingCard(false)} block color="danger" className="mt-3">
                    Cancel
                  </Button>
                </Col>}
              <Col>
                <Button id={customerCard ? 'saveNewCard' : 'addCardToAccount'} title={customerCard ? 'Save New Card' : 'Add Card To Account'} disabled={formState.submitted || !formData.card || !formData.expire || !formData.cvc || !formData.postalCode || !stripe || !elements} onClick={() => setFormState({
              submitted: true
            })} block className="mt-3" color="success">
