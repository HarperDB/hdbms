...
function EditDelete() {
  ...
  const deleteUser = async () => {
    if (formData.deleteUsername !== 'DELETE') {
      ...
      if (response.message.indexOf('successfully') !== -1) {
        ...
        setFormState({});
        setTimeout(() => navigate(pathname.replace(`/${userId}`, '')), 100);
      } else {
  ...
  return <ErrorBoundary onError={(error, componentStack) => addError({
    ...
    customerId
      ...
      <Row>
        ...
        <Col xs="4">
          <Input id="delete_username" onChange={e => setFormData({
          deleteUsername: e.target.value
        })} type="text" className="text-center" title="confirm username to delete" placeholder={`Enter "DELETE" here to enable deletion.`} value={formData.deleteUsername || ''} />
        </Col>
        <Col xs="4">
          <Button id="deleteUser" block color="danger" onClick={deleteUser} disabled={formData.deleteUsername !== 'DELETE' || formState.submitted}>
