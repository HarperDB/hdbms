...
function LocalApp() {
  ...
  return <div id="local-studio" className={`${persistedUser.theme}`}>
      <div id="app-container">
        <Suspense fallback={<Loader header=" " spinner />}>
          <TopNav loggedIn={instanceAuths?.local?.valid} />
        </Suspense>
        {fetchingUser ? <Loader header="signing in" spinner /> : instanceAuths?.local?.valid ? <ErrorBoundary FallbackComponent={ErrorFallback}>
            <Suspense fallback={<Loader header=" " spinner />}>
      ...
      <div id="app-bg-color" />
      <div className="version">HarperDB Local Studio v{config.studioVersion}</div>
    </div>;
}
