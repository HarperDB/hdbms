...
class JSONInput extends Component {
    ...
    updateInternalProps(){
        let colors = {}; let style = {}; let theme = themes.dark_vscode_tribute;
    ...
    render(){
        const
            ...
            const {colors} = this;
            const {style} = this;
            const {confirmGood} = this;
            const {totalHeight} = this;
            const {totalWidth} = this;
            const hasError     = !!this.props.error || (error ? 'token' in error : false);
        this.renderCount++;
        return (
            <div
                name  = 'outer-box'
    ...
    tokenize(something){
        ...
        if('nodeType' in something){
            ...
            for(var i = 0; i < buffer.tokens_unknown.length; i++){
                const token = buffer.tokens_unknown[i];
            ...
            for(var i = 0; i < buffer.tokens_proto.length; i++){
                const token = buffer.tokens_proto[i];
            ...
            for(var i = 0; i < buffer.tokens_split.length; i++){
                const token = buffer.tokens_split[i];
                let
                    {type} = token;
                    const {string} = token;
                    const {length} = string;
                ...
                switch(type){
                    ...
                    case 'string' :
                        const
                            ...
                            const quote_primary = quotes.indexOf(firstChar);
        ...
        if(!('nodeType' in something)){
            ...
            for(var i = 0; i < buffer.inputText.length; i++){
                ...
                const
                    a = determine_value();
                    const b = determine_string();
                    const c = escape_character();
                if(!a&&!b&&!c) if(!buffer.stringOpen) buffer.tokenSecondary += buffer.currentChar;
            }
            const buffer2 = { brackets : [], isValue : false, tokens: [] };
            ...
            for(var i = 0; i < buffer2.tokens.length; i++){
                const token =  buffer2.tokens[i];
                const span = newSpan(i,token,token.depth);
                switch(token.string){
                    case '{' : case '[' :
                        const nextToken = i < (buffer2.tokens.length - 1) - 1 ? buffer2.tokens[i+1] : '';
                        if('}]'.indexOf(nextToken.string)===-1)
