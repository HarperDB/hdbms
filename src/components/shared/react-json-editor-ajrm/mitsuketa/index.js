...
function deepRemoveAll_Key(identity,keyName,maxDepth){
    ...
    const clonedIdentity = deepClone(identity);
    const paths = locateAll_Key(clonedIdentity,keyName,maxDepth);
...
function deepGet_Key(collection,keyName,maxDepth=null){
    if(getType(keyName)!=='string') return undefined;
    if(keyName==='') return undefined;
    let path = locate_Key(collection,keyName,maxDepth);
    if(path === false) return undefined;
    if(path === '') path = keyName; else path += (`.${  keyName}`);
    path = path.split('.');
    let result = collection;
...
function matchDepth(collection,identity,maxDepth=null){
    let path = locate(collection, identity, maxDepth);
...
function countMatches(collection,identity,nthDepth=null,maxDepth=null){
    let
        depth;
        const nthDepth_isNull = nthDepth === null;
        const maxDepth_isNull = maxDepth === null;
    ...
    const paths = locateAll(collection,identity,depth);
...
function deepFilter(collection, identity, maxDepth=null){
    const paths = locateAll(collection, identity, maxDepth);
    if(paths === false) return undefined;
    const results = paths.map(path => {
...
function identical(identityA,identityB){
    ...
    const Keys = Object.keys(identityA); const KeyCount = Keys.length;
    let childMatch = true;
    for(let i = 0; i < KeyCount; i++) {
        const Key = Keys[i]; const identicalMatch = identical(identityA[Key],identityB[Key]);
...
const mitsuketa = {
    getType(identity)                                          { return getType(identity);                                                     },
    sameType(identityA,identityB)                               { return sameType(identityA,identityB);                                         },
    sameStructure(identityA,identityB)                               { return sameStructure(identityA,identityB);                                    },
    identical(identityA,identityB)                               { return identical(identityA,identityB);                                        },
    isIterable(identity)                                          { return isIterable(identity);                                                  },
    containsKeys(identity,keyList)                                  { return containsKeys(identity,keyList);                                        },
    trim(identity,keyList)                                  { return trim(identity,keyList);                                                },
    locate(collection,identity,maxDepth)                      { return locate(collection,identity,maxDepth);                                  },
    deepGet(collection,identity,maxDepth)                      { return deepGet(collection,identity,maxDepth);                                 },
    locateAll(collection,identity,maxDepth)                      { return locateAll(collection,identity,maxDepth);                               },
    deepFilter(collection,identity,maxDepth)                      { return deepFilter(collection,identity,maxDepth);                              },
    exists(collection,identity,maxDepth)                      { return exists(collection,identity,maxDepth);                                  },
    onlyExisting(collection,identities,maxDepth)                    { return onlyExisting(collection,identities,maxDepth);                          },
    onlyMissing(collection,identities,maxDepth)                    { return onlyMissing(collection,identities,maxDepth);                           },
    length(identity)                                          { return length(identity);                                                      },
    isFalsy(identity)                                          { return isFalsy(identity);                                                     },
    isTruthy(identity)                                          { return isTruthy(identity);                                                    },
    foundTruthy(collection,identity,maxDepth)                      { return foundTruthy(collection,identity,maxDepth);                             },
    onlyTruthy(collection,identities,property,maxDepth)           { return onlyTruthy(collection,identities,property,maxDepth);                   },
    foundFalsy(collection,identity,maxDepth)                      { return foundFalsy(collection,identity,maxDepth);                              },
    onlyFalsy(collection,identities,property,maxDepth)           { return onlyFalsy(collection,identities,property,maxDepth);                    },
    countMatches(collection,identity,nthDepth,maxDepth)             { return countMatches(collection,identity,nthDepth,maxDepth);                   },
    matchDepth(collection,identity,maxDepth)                      { return matchDepth(collection,identity,maxDepth);                              },
    maxDepth(identity,maxLayer)                                 { return maxDepth(identity,maxLayer);                                           },
    locate_Key(collection,keyName,maxDepth)                       { return locate_Key(collection,keyName,maxDepth);                               },
    deepGet_Key(collection,keyName,maxDepth)                       { return deepGet_Key(collection,keyName,maxDepth);                              },
    locateAll_Key(collection,keyName,maxDepth)                       { return locateAll_Key(collection,keyName,maxDepth);                            },
    deepFilter_Key(collection,keyName,maxDepth)                       { return deepFilter_Key(collection,keyName,maxDepth);                           },
    deepClone(identity,maxDepth,startDepth)                      { return deepClone(identity,maxDepth,startDepth);                               },
    renameKey(identity,keyName,newKeyName,maxDepth)              { return renameKey(identity,keyName,newKeyName,maxDepth);                       },
    renameKeys(identity,keyName,newKeyName,maxDepth)              { return renameKeys(identity,keyName,newKeyName,maxDepth);                      },
    deepRemove_Key(identity,keyName,maxDepth)                         { return deepRemove_Key(identity,keyName,maxDepth);                             },
    deepRemoveAll_Key(identity,keyName,maxDepth)                         { return deepRemoveAll_Key(identity,keyName,maxDepth);                          }
}
