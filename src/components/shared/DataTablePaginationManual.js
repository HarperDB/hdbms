...
// Our table component
function DataTablePaginationManual({ page, pageSize, totalPages, onPageChange, onPageSizeChange }) {
  return <Row className="pagination">
    <Col xs="12" sm="2" className="previous">
      <Button className="mb-2" color="purple" block onClick={() => onPageChange(page - 1)} disabled={!totalPages || page === 0}>
    ...
    <Col xs="12" sm="4" className="paginator">
      ...
      <div className="page-count">&nbsp;/&nbsp;{totalPages || <i className="fa fa-spinner fa-spin" />}</div>
    </Col>
    ...
    <Col xs="12" sm="2" className="next">
      <Button className="mb-2 pull-right" block color="purple" onClick={() => onPageChange(page + 1)} disabled={!totalPages || page + 1 === totalPages}>
