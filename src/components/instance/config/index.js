...
function ConfigIndex() {
	...
	const unusedCompute = useStoreState(
		appState,
		(s) =>
			s.subscriptions[is_local ? 'local_compute' : 'cloud_compute']?.filter(
	...
	const unusedStorage = useStoreState(
		...
		(s) =>
			s.subscriptions?.cloud_storage?.filter(
				(p) => !p.value.storage_subscription_name || p.value.storage_quantity_available
			) || []
	);
						...
						<Col className="text-end">
							{(!!compute_subscription_id || !!unusedCompute.length) && (
								<span className="floating-card-header">
									prepaid:{' '}
									<Button
										id="showPrepaidCompute"
										color="link"
										onClick={() => setShowPrepaidCompute(!showPrepaidCompute)}
									>
						...
						<Col className="text-end">
							{(!!storage_subscription_id || !!unusedStorage.length) && (
								<span className="floating-card-header">
									prepaid:{' '}
									<Button
										id="showPrepaidStorage"
										color="link"
										onClick={() => setShowPrepaidStorage(!showPrepaidStorage)}
									>
