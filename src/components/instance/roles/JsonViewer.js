...
function JsonViewer({
  ...
  const icons = {
    checked: <div style={{
      width: '100%',
      textAlign: 'center'
    }}>manage {versionAsFloat >= 4.2 ? 'databases' : 'schemas'}/tables</div>,
    unchecked: <div style={{
      width: '100%',
      textAlign: 'center'
    }}>manage {versionAsFloat >= 4.2 ? 'databases' : 'schemas'}/tables</div>
  ...
  useAsyncEffect(async () => {
    if (roleId && roles && roles.find(r => r.id === roleId)?.permission) {
      const currentRolePermissions = roles.find(r => r.id === roleId)?.permission;
      const defaultStructurePermissions = Array.isArray(currentRolePermissions.structureUser) ? currentRolePermissions.structureUser.join(', ') : currentRolePermissions.structureUser || false;
  ...
  }, [roleId, roles, lastUpdate, showAttributes]);
  const submitRecord = async e => {
    ...
    if (hasStructureUser) {
      permission.structureUser = newStructureUser === true ? true : newStructureUser ? newStructureUser.split(/[ ,]+/).filter(v => v !== '') : false;
    }
    ...
    setLoading(false);
    if (response.error) {
      let message = response.roleErrors || response.message;
  ...
  return <ErrorBoundary onError={(error, componentStack) => addError({
      ...
      {hasStructureUser && <>
          <Row>
            <Col xs="12" sm="6" md="12" lg="5" xl="4" className="pt-2 pb-2">
              <ToggleButton width="100%" checked={!!newStructureUser} onChange={() => {
            setNewStructureUser(newStructureUser ? false : newStructureUser === false && activeStructureUser ? activeStructureUser : true);
            setChanged(true);
          }} icons={icons} />
            </Col>
            {newStructureUser && <Col xs="12" sm="6" md="12" lg="7" xl="8">
                <Input onChange={e => {
            setNewStructureUser(e.target.value ? e.target.value : true);
            setChanged(true);
          }} type="text" id="permitted_schemas" title={`permitted ${versionAsFloat >= 4.2 ? 'databases' : 'schemas'}`} placeholder={`comma-separated ${versionAsFloat >= 4.2 ? 'databases' : 'schemas'}, or blank for all`} value={newStructureUser === true ? '' : newStructureUser} disabled={loading} />
              </Col>}
          </Row>
          <hr />
        </>}

      <JSONInput placeholder={activePermissions} height={jsonHeight} theme="light_mitsuketa_tribute" colors={{
    ...
    }} locale={locale} width="100%" waitAfterKeyPress={1000} confirmGood={false} onChange={value => {
