...
function DashboardChart({
  ...
  const canDelete = useStoreState(appState, s => s.auth?.userId === userId);
  const [chartData, setChartData] = useState(false);
  const [loading, setLoading] = useState(true);
  const [deleting, setDeleting] = useState(false);
  const options = chartData && !chartData.error && chartOptions({
    title: name,
    type,
    labels: chartData.map(d => d[labelAttribute]),
    theme
  });
  const series = !chartData || chartData.error ? [] : type === 'single value' ? chartData[0][seriesAttributes[0]] : ['donut', 'pie'].includes(type) ? chartData.map(d => d[seriesAttributes[0]]) : seriesAttributes.map(s => ({
    name: s,
  ...
  useAsyncEffect(() => getChartData(), () => controller && controller.abort(), []);
  useInterval(getChartData, config.refreshContentInterval);
  const handleRemoveChart = chartId => {
    setDeleting(true);
    removeChart(chartId);
  };
  return chartData.error ? <Col lg="6" xs="12" className="mb-3" key={name}>
      <Card className="dashboard-chart">
        <CardBody className="text-nowrap position-relative">
          {confirmDelete === id ? <ButtonGroup size="sm" className="chart-remove-confirm">
            ...
            </ButtonGroup> : <Button title="Remove this chart" className="chart-remove" color="link" onClick={() => setConfirmDelete(id)}>
    ...
    </Col> : <Col lg="6" xs="12" className="mb-3" key={name}>
      <Card className="dashboard-chart">
        <CardBody className="text-nowrap position-relative">
          {loading && <Button disabled title="loading" className="chart-loading" color="link">
          ...
          {confirmDelete === id ? <ButtonGroup size="sm" className="chart-remove-confirm">
            ...
            </ButtonGroup> : canDelete ? <Button title="Remove this chart" className="chart-remove" color="link" onClick={() => setConfirmDelete(id)}>
              <i className="fa fa-times text-darkgrey" />
            </Button> : null}
          {chartData && type === 'single value' ? <div className="dashboard single-value-chart">
              <div className="title">{name}</div>
              <h1>{isNumeric(series) ? series.toFixed(2) : series}</h1>
            </div> : chartData ? <Chart options={options} series={series} type={type} height={220} /> : null}
        </CardBody>
      </Card>
    </Col>;
}
