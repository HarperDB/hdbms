...
function EditDelete() {
  ...
  const deleteUser = async () => {
    if (formData.deleteUsername !== username) {
  ...
  return <ErrorBoundary onError={(error, componentStack) => addError({
    error: {
      ...
      <Row>
        ...
        })} type="text" className="text-center" title="confirm username to delete" placeholder={`Enter "${username}" here to enable deletion.`} value={formData.deleteUsername || ''} />
        </Col>
        <Col xs="4">
          <Button id="deleteUser" block color="success" onClick={deleteUser} disabled={username !== formData.deleteUsername || formState.submitted}>
