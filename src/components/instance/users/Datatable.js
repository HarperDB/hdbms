...
function Datatable({
  ...
  const rowClick = useCallback(userId => {
    navigate(`/o/${customerId}/i/${computeStackId}/users/${userId}`);
  },
  // eslint-disable-next-line react-hooks/exhaustive-deps
  [computeStackId, customerId]);
  useEffect(() => {
    let isMounted = true;
    ...
    error: {
      ...
      <Row className="floating-card-header">
        ...
        <Col className="text-end">
          ...
          <Button color="link" title="Filter Users" className="me-2" onClick={() => setTableState({
          ...tableState,
      ...
      <Card className="mt-3">
        <CardBody className="react-table-holder">
          <DataTable columns={tableState.dataTableColumns} data={tableState.tableData} currentPage={tableState.page} pageSize={tableState.pageSize} totalPages={tableState.totalPages} showFilter={tableState.showFilter} sorted={tableState.sorted} loading={loading} onFilteredChange={value => setTableState({
          ...tableState,
          filtered: value
