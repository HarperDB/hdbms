...
export default function DeleteFolderWindow({ active, selectedFolder, onConfirm, onCancel }) {
  ...
  return (
    <div className="content-window">
      <h4 className="mb-5">
        Delete <span className="text-danger">{isProjectFolder ? selectedFolder?.project : projectSubdir}</span> {isProjectFolder ? 'Project' : 'Folder'}
      </h4>
      ...
      <div className="mt-5">
        {isProjectFolder ? (
          ...
          <div>
            Are you sure you want to delete the folder <span className="text-danger">{projectSubdir}</span> from <span className="text-danger">{selectedFolder?.project}</span>?
          </div>
        )}
      </div>
    </div>
