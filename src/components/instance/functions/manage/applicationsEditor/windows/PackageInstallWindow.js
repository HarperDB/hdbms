...
export default function PackageInstallWindow({
  ...
  function updateSelectedPackageType({
    value
  }) {
  ...
  return !active ? null : <div className="content-window">
      <h4 className="mb-5">Import Remote App Package</h4>

      <Input valid={projectNameIsValid && projectName.length > 0} title="enter a name for this package" value={projectName} placeholder="Application Name" onChange={e => {
      setProjectName(e.target.value);
      setProjectNameIsValid(isValidProjectName(e.target.value));
    }} />

      {!config.isLocalStudio && <Select className="react-select-container package-install-deploy-targets-dropdown mt-2" classNamePrefix="react-select" isSearchable isMulti placeholder="Choose your deploy targets" onChange={selected => {
      ...
      <Select className="react-select-container mt-2" classNamePrefix="react-select" placeholder="How would you like to fetch this package?" onChange={updateSelectedPackageType} options={[{
      ...
      {!projectNameIsValid && projectName.length > 0 && <span className="text-danger text-center mt-3">
      ...
      {projectNameIsValid && deployTargets.length > 0 && packageType ? <>
        ...
        setLoading(false);
      }} className={cn('btn btn-block btn-success mt-3', {
        loading
