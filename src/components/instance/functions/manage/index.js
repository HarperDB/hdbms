...
function getDeployTargets(instanceList, instanceAuthList, thisCsId, auth) {
  return instanceList.reduce((memo, instance) => {
    const csId = instance.computeStackId;
...
function ManageIndex({
  ...
  const {
    fileTree
  } = useStoreState(instanceState, s => s.customFunctions);
  ...
  function removeFileFromLocalStorage({
    path
  }) {
    const updatedCache = {
      ...editorCache
    };
    const fileKey = `${computeStackId}_${path}`;
  ...
  async function restartWithLoadingState({
    auth: instanceAuth,
    url: instanceUrl
  }) {
  ...
  async function selectNewFile(selectedFile) {
    ...
    const fileCacheKey = `${computeStackId}_${path}`;
  ...
  return supportsApplicationsAPI ? <ApplicationsEditor fileTree={fileTree} deployTargets={getDeployTargets(instances, instanceAuths, computeStackId, auth)} onRevertFile={revertFileChanges} onFileChange={saveFileToLocalStorage} onFileSave={saveFileToInstance} onUpdate={refreshCustomFunctions} onAddFile={createNewFile} onAddProject={createNewProject} onAddProjectFolder={createNewProjectFolder} onInstallPackage={installPackage} onDeployProject={deployProject} onDeleteFile={deleteFile} onDeleteFolder={deleteFolder} onDeletePackage={deletePackage} onFileSelect={selectNewFile} onFileRename={renameFile} onFolderRename={renameFolder} refreshingCustomFunctions={loading} restartInstance={async () => restartWithLoadingState({
    auth,
    url
  })} restartingInstance={restartingInstance} /> : <Index refreshCustomFunctions={refreshCustomFunctions} loading={loading} />;
}
