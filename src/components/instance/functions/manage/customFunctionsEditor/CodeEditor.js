...
function CodeEditor() {
  ...
  const setEditorToFile = useCallback(async () => {
    if (project && type && file && file !== 'undefined') {
      const endpointCode = await getCustomFunction({
        auth,
        url,
        project,
        type,
      ...
      setCode(endpointCode?.message);
      setOriginalCode(endpointCode?.message);
    } else {
      setCode('');
    }
  }, [auth, url, project, type, file, setCode]);
  const handleSubmit = async () => {
    const response = await setCustomFunction({
      auth,
      url,
  ...
  useEffect(() => {
    setEditorToFile();
  }, [project, file, setEditorToFile, computeStackId]);
  return <>
      <Row className="floating-card-header">
        ...
        <Col className="text-end">
          ...
          <Button onClick={() => navigate(`/o/${customerId}/i/${computeStackId}/functions/deploy/${project}`)} color="link" className="me-2">
