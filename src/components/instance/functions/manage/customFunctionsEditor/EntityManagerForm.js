...
function EntityManagerForm({
  ...
  const createItem = async e => {
    ...
    if (itemType === 'projects') {
      ...
      const functionContent = generateFunctionTemplate(itemType);
  ...
  return <Row className="item-row form">
      ...
      }} value={entityName || ''} disabled={addingItem} type="text" name="name" placeholder={itemType === 'projects' ? 'ex: api, api-v1, dogs' : itemType === 'routes' ? 'ex: v1, post, public' : 'ex: filter, auth, queries'} />
      </Col>
      <Col className="item-action">
        {addingItem ? <Button color="success" className="round me-1">
