...
function JSONEditor({
  ...
  const baseUrl = `/o/${customerId}/i/${computeStackId}/browse/${schema}/${table}`;
  ...
  const initializeEditorContent = async () => {
    if (action === 'edit') {
      ...
      if (rowData) {
        const hashAttribute = rowData[hashAttribute];
        const createdtime = rowData._Createdtime__; // eslint-disable-line no-underscore-dangle
        const updatedtime = rowData._Updatedtime__; // eslint-disable-line no-underscore-dangle

        delete rowData._Createdtime__; // eslint-disable-line no-underscore-dangle
        delete rowData._Updatedtime__; // eslint-disable-line no-underscore-dangle
        delete rowData[hashAttribute];
        const newCurrentValue = {
          [hashAttribute]: hashAttribute,
          ...rowData,
          _Createdtime__: createdtime,
          _Updatedtime__: updatedtime
        };
  ...
  const deleteRecord = async e => {
    ...
    return setTimeout(() => navigate(`/o/${customerId}/i/${computeStackId}/browse/${schema}/${table}`), 100);
  };
      ...
      <Card className="my-3">
        <CardBody>
          ...
          <Card className="mb-2 json-editor-holder">
            <CardBody>
              <Editor height="350px" language="json" value={currentValue} theme="vs-dark" onValidate={markers => {
          ...
          <Row>
            <Col md="4" className="mt-2">
              {confirmDelete ? <div className="pt-2">Delete this record?</div> : <Button disabled={saving || deleting} id="backToTable" block color="black" onClick={() => navigate(`/o/${customerId}/i/${computeStackId}/browse/${schema}/${table}`)}>
