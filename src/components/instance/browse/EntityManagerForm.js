...
function EntityManagerForm({
  ...
  const createItem = async e => {
    ...
    if (itemType === 'table') {
      ...
      operation.hashAttribute = hashAttribute;
    } else {
  ...
  return <Row className="item-row form">
      <Col className="input-holder">
        <Input id="name" invalid={nameError} onChange={e => {
        toggleNameError(false);
        setEntityName(e.target.value);
      }} value={entityName || ''} disabled={addingItem} type="text" name="name" placeholder="name" />
      </Col>
      {itemType === 'table' && <Col className="input-holder">
          <Input id="hash_attribute" invalid={hashError} disabled={addingItem} onChange={e => {
        toggleHashError(false);
        setHashAttribute(e.target.value);
      }} value={hashAttribute || ''} type="test" name="hash_attribute" placeholder="hash attr." />
        </Col>}
      <Col className="item-action">
        {addingItem ? <Button id="creatingItem" color="success" className="round">
