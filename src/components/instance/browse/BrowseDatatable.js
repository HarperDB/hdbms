...
function BrowseDatatable({
  ...
  useInterval(() => tableState.autoRefresh && setLastUpdate(Date.now()), config.refreshContentInterval);
  return <>
      <DataTableHeader totalRecords={totalRecords} loading={loading} loadingFilter={loadingFilter} autoRefresh={tableState.autoRefresh} refresh={() => setLastUpdate(Date.now())} toggleAutoRefresh={() => setTableState({
      ...tableState,
      autoRefresh: !tableState.autoRefresh
    })} toggleFilter={() => setTableState({
      ...tableState,
      showFilter: !tableState.showFilter
    })} />
      <Card className="my-3">
        <CardBody className="react-table-holder">
          <DataTable manual columns={tableState.dataTableColumns || []} data={tableState.tableData || []} currentPage={tableState.page} pageSize={tableState.pageSize} totalPages={totalPages || 0} showFilter={tableState.showFilter} sorted={tableState.sorted.length ? tableState.sorted : [{
          id: tableState.hashAttribute,
          desc: false
        }]} loading={loading && !tableState.autoRefresh} onFilteredChange={value => {
          setTableState({
            ...tableState,
            page: 0,
            filtered: value
          ...
          const recordViewUrl = `/o/${customerId}/i/${computeStackId}/browse/${encodedSchema}/${encodedTable}/edit/${encodedHash}`;
