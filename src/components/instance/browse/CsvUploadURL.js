...
function CsvUploadURL() {
    ...
    try {
      ...
      if (status === 'ERROR') {
        if (['Error: CSV Load failed from URL', 'Error downloading CSV file'].some(i => message.indexOf(i) !== -1)) {
          return setFormState({
            error: 'The URL did not return a valid csv file'
          });
        }
        if (['unsupported content type'].some(i => message.indexOf(i) !== -1)) {
          return setFormState({
            error: `File error: ${message.split(',')[1]}`
          });
        }
        return setFormState({
  ...
  useAsyncEffect(async () => {
    if (formState.submitted) {
      if (isURL(formData.csvUrl)) {
        setFormState({
          uploading: true
        });
        const uploadJob = await csvURLLoad({
          schema,
          table,
  ...
  return <ErrorBoundary onError={(error, componentStack) => addError({
    error: {
      ...
      <Row>
        ...
        <Col xs="12" lg="4">
          {formState.error ? <div className="text-danger csv-status">
              <i className="fa fa-exclamation-triangle me-3" />
              {formState.error}
            </div> : formState.uploading ? <div className="csv-status">
              <i className="fa fa-spin fa-spinner me-3" />
              uploading .csv into {schema}.{table}
            </div> : <Input onChange={e => setFormData({
          csvUrl: e.target.value
        })} type="text" invalid={formData.csvUrl && !isURL(formData.csvUrl)} id="csv_url" title="csv url" placeholder="CSV file URL" value={formData.csvUrl || ''} disabled={formState.submitted} />}
        </Col>
        <Col xs="12" lg="4">
          {formState.error ? <Button id="clearURL" block color="danger" onClick={() => {
          setFormState({});
          setFormData({});
        }}>
              Clear URL
            </Button> : <Button id="importFromURL" disabled={!!Object.keys(formState).length || !isURL(formData.csvUrl)} block color="success" onClick={() => setFormState({
          submitted: true
        })}>
              Import From URL
            </Button>}
