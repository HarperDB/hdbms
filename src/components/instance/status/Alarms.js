...
function Alarms() {
  ...
  const alarms = useStoreState(appState, s => s.alarms && s.alarms[computeStackId]?.alarms, [computeStackId]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      await getAlarms({
        ...
        customerId,
        currentAlarmsLength: alarms?.length
      });
      ...
      <Card className="my-3">
        <CardBody className="item-list">
          ...
          <div className="item-scroller">
            {loading && !alarms && !autoRefresh ? <div className="pt-5 text-center">
                <i className="fa fa-spinner fa-spin text-lightgrey" />
              </div> : alarms?.length ? alarms.map(a => <AlarmsRow key={a.id} {...a} />) : <div className="pt-5 text-center">no alarms found</div>}
          </div>
        </CardBody>
      </Card>
      <br />
