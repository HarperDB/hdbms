...
function Logs() {
  ...
  const logs = useStoreState(instanceState, s => s.logs, [computeStackId]);
  ...
  useInterval(() => auth && autoRefresh && setLastUpdate(Date.now()), config.refreshContentInterval);
      ...
      <Card className="my-3">
        <CardBody className="item-list">
          ...
          <div className="item-scroller">
            {loading && !logs && !autoRefresh ? <div className="pt-5 text-center">
                <i className="fa fa-spinner fa-spin text-lightgrey" />
              </div> : logs && logs.length ? logs.map((l, i) =>
          // eslint-disable-next-line react/no-array-index-key
          <LogRow key={i} {...l} />) : logs && !logs.length ? <div className="pt-5 text-center">no logs found</div> : <div className="pt-5 text-center">no logs found in this view</div>}
          </div>
        </CardBody>
      </Card>
      <br />
