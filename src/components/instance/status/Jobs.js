      ...
      <Card className="my-3">
        <CardBody className="item-list">
          <Row className="header">
            ...
            {!loading && jobsError ? <Col xs="6" className="text-end text-danger text-bold text-nowrap">
          ...
          <div className="item-scroller">
            {loading && !jobs && !autoRefresh ? <div className="pt-5 text-center">
                <i className="fa fa-spinner fa-spin text-lightgrey" />
              </div> : jobs?.length ? jobs.map(j => <JobRow key={j.id} {...j} />) : <div className="pt-5 text-center">no jobs found</div>}
          </div>
        </CardBody>
      </Card>
      <br />
