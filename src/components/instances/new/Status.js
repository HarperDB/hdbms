...
function Status({
  ...
  useAsyncEffect(async () => {
    addTCAcceptance({
      auth,
      ...auth,
      tcVersion: newInstance.tcVersion,
      customerId: newInstance.customerId
    });
    ...
    if (response.error) {
      ...
      const returnedComputeStackId = response.computeStackId || response.instanceId;
      const updatedInstanceAuths = {
        ...instanceAuths,
        [returnedComputeStackId]: {
          user: newInstance.user,
          pass: newInstance.pass,
          super: newInstance.super
        }
      };
  ...
  return formState.error ? <Card>
      ...
      <CardBody>
        <div className="p-4 text-center">
          <b>{newInstance.isLocal ? 'Adding' : 'Creating'} Your Instance</b>
          <br />
          <br />
          <br />
