...
function DetailsCloud() {
	...
	const unusedCompute = useStoreState(
		appState,
		(s) => s.subscriptions?.cloud_compute?.filter((p) => p.value.active && p.value.compute_quantity_available) || []
	);
	const unusedStorage = useStoreState(
		appState,
		(s) =>
			s.subscriptions?.cloud_storage?.filter(
				(p) => p.value.active && p.value.storage_quantity_available >= p.value.data_volume_size
			) || []
	);
	...
	const isFree =
		!formData.compute_price &&
		!formData.compute_subscription_id &&
		!formData.storage_price &&
		!formData.storage_subscription_id;
	const needsCard = products && storage && !hasCard && !isFree && !is_unpaid;
	const totalFreeCloudInstances = orgs
		.filter((o) => user_id === o.owner_user_id)
		...
		<>
				...
				<CardBody>
					...
					>
						{!products ? (
							...
							<RadioCheckbox
								...
								options={products}
								defaultValue={
									newInstance.stripe_plan_id
										? products.find((p) => p.value.stripe_plan_id === newInstance.stripe_plan_id)
										: products[0]
								}
							/>
						) : (
					...
					{storage && (
						<ContentContainer
							...
							subheader={
								<DetailsSubheader
									hasPrepaid={unusedStorage.length}
									newInstance={newInstance}
									setNewInstance={setNewInstance}
									toggleValue="showPrepaidStorage"
								/>
							}
							maxHeight="120px"
						>
							<RadioCheckbox
								...
								options={storage}
								defaultValue={
									newInstance.data_volume_size
										? storage.find((p) => p.value.data_volume_size === newInstance.data_volume_size)
										: storage[0]
								}
							/>
						</ContentContainer>
							...
							<RadioCheckbox
								...
								defaultValue={
									newInstance.instance_region
										? regions.find((p) => p.value === newInstance.instance_region)
										: regions[0]
								}
							/>
						) : (
			...
			<Row>
				<Col sm="6">
					<Button
						id="backToBasicInfo"
						onClick={() => navigate(`/o/${customer_id}/instances/new/meta_cloud`)}
