...
function CardBackLogin({
  ...
  useAsyncEffect(async () => {
    ...
    if (submitted) {
      ...
      if (!user || !pass) {
        ...
        if (isSsl && isLocal && result.error && result.type === 'catch') {
          setFormState({
            error: 'SSL ERROR. ACCEPT SELF-SIGNED CERT?',
            url
          });
        } else if (result.error && wavelengthZoneId && result.type === 'catch') {
          setFormState({
            error: "Can't reach Wavelength instance. On Verizon?",
            url: false
          });
        } else if (result.error && !isSsl && isLocal && result.type === 'catch') {
          setFormState({
            error: "Can't reach non-SSL instance. Enable SSL?",
            url: 'https://harperdb.io/developers/documentation/security/configuration/'
          });
        } else if ((result.error && result.message === 'Login failed' || result.error === 'Login failed') && !isLocal) {
  ...
  return <ErrorBoundary onError={(error, componentStack) => addError({
    error: {
      ...
      <CardBody>
            {formState.error && formState.error.indexOf('This instance was recently') !== -1 ? <CardInstanceUpdateRole formState={formState} setFormData={setFormData} setFormState={setFormState} /> : <Form>
                <Input id="username" autoComplete="username" onChange={e => setFormData({
            ...formData,
            user: e.target.value
          })} className="text-center mb-1" type="text" title="instance user" placeholder="instance user" disabled={formState.submitted} onKeyDown={keyDown} />
                <Input id="password" name="password" autoComplete="current-password" onChange={e => setFormData({
            ...formData,
            pass: e.target.value
          })} className="text-center mb-2" type="password" title="instance password" placeholder="instance password" disabled={formState.submitted} onKeyDown={keyDown} />
                <Row className="g-0">
                ...
                setFormState({});
                setFlipState(false);
              }} title="Cancel" block color="grey" disabled={formState.submitted}>
                      Cancel
                    </Button>
                  </Col>
                ...
                submitted: true
              })} title="Log Into Instance" block color="success" disabled={formState.submitted}>
