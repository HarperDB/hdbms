...
function ValidatedRoute(auth) {
  ...
  if (auth.auth.orgs.some(o => o.customerId === customerId)) {
    return <Outlet />;
  }
...
function App() {
  ...
  useEffect(() => {
    setShowVersionAlert(checkVersion({
      apiVersion: version.studio
    }));
  }, [version.studio]);
  useEffect(() => {
    if (auth?.email) {
      if (window.Kmq) {
        window.Kmq.push(['identify', auth.email]);
      }
  ...
  return <div className={`${theme} ${config.maintenance ? 'maintenance' : ''}`}>
      <div id="app-container">
        <Suspense fallback={<Loader header=" " spinner />}>
          <TopNav isMaintenance={isMaintenance} loggedIn />
        </Suspense>
        {fetchingUser ? <Loader header="signing in" spinner /> : loggedIn ? <ErrorBoundary FallbackComponent={ErrorFallback}>
            <Suspense fallback={<Loader header=" " spinner />}>
          ...
          </ErrorBoundary> : <ErrorBoundary FallbackComponent={ErrorFallbackAuth}>
            <Suspense fallback={<Loader header=" " spinner />}>
              <Routes>
      ...
      <div className="version">
        HarperDB Studio v{config.studioVersion}
